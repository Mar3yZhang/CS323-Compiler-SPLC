        ...
        case 3: {
            ...
            if (opt == "ASSIGN") {
                translate_exp_assign_exp(Exp, reg);
                       ...
                       || opt == "MUL" || opt == "DIV") {
                translate_exp_plus_exp(Exp, reg);
            }
            break;
        }
    }
...
//     if (!reg_has_int(content)) {
    TAC *tac = new TAC(reg, Exp->child[0]->content, "", TAC_TYPE::ASSIGN_INT);
...
void translate_exp_id(Node *Exp, const string &reg) {
    ...
    if (param_id_reg_mapper.count(id) != 0) {  //是传参的寄存器对应的值，不需要额外处理
        TAC *tac = new TAC(reg, param_id_reg_mapper[id], "", TAC_TYPE::ASSIGN_INT);
        ir_tac.push_back(tac);
    }
