...
const auto caller = [](unordered_set<string> setOfVaris, vector<string> args,
    ...
    unordered_map<string, int> varitoInt{param_to_reg};
    begin_strs.push_back(string("    addi $sp, $sp, -").append(to_string((4 + setOfVaris.size() + 1) * 4)));
    ...
    for (const auto &item: setOfVaris) {
        ...
        begin_strs.push_back(string("    sw $t0, ").append(to_string((order++) * 4).append("($sp)")));
        end_strs.push_back(string("    lw $t0,").append(to_string((order - 1) * 4).append("($sp)")));
        end_strs.push_back(string("    sw $t0,_").append(item));
        if (varitoInt.count(item) == 0) {
            varitoInt[item] = order - 1;
        }
    }
    begin_strs.push_back(string("    sw $ra, ").append(to_string((order++) * 4).append("($sp)")));
    end_strs.push_back(string("    lw $ra, ").append(to_string((order - 1) * 4).append("($sp)")));
    end_strs.push_back(string("    addi $sp, $sp, ").append(to_string((4 + setOfVaris.size() + 1) * 4)));
    for (auto i = 0u; i < args.size(); i++) {
        begin_strs.push_back(string("    lw $a")
                                     .append(to_string(args.size() - 1 - i)).append(",")
                                     .append(to_string(varitoInt[args[i]] * 4)).append("($sp)"));
    }
    string will_return_begin;
    string will_return_end;
    ...
    return pair<string, string>{will_return_begin, will_return_end};
};